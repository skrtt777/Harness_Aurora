"""Bounded supervised LoRA training. Only assistant tokens contribute to loss.
Original base remains immutable; output contains actual trained adapter tensors.
"""
...
parser.add_argument('--run', default='aurora-lora-v1')
...
def encode(path):
    ...
    return rows
train=encode(root/'dataset-v1/train.jsonl'); dev=encode(root/'dataset-v1/dev.jsonl')
def batch(rows):
...
manifest={'run':args.run,'base':json.loads((root/'base-provenance.json').read_text()),'seed':20260920,'epochs':args.epochs,'rank':16,'alpha':32,'learningRate':.0001,'batchSize':1,'gradientAccumulation':8,'maxSequenceLength':2048,'trainableParameters':trainable,'totalParameters':total,'trainHash':sha(root/'dataset-v1/train.jsonl'),'devHash':sha(root/'dataset-v1/dev.jsonl'),'trainingScriptHash':sha(Path(__file__)),'gpu':torch.cuda.get_device_name(0),'torch':torch.__version__,'startedAt':time.time()}
(out/'manifest.json').write_text(json.dumps(manifest,indent=2))
(out/'packages.txt').write_text(subprocess.check_output([__import__('sys').executable,'-m','pip','freeze'],text=True))
optim=torch.optim.AdamW((p for p in model.parameters() if p.requires_grad),lr=.0001,weight_decay=.01)
...
for epoch in range(args.epochs):
    ...
    for offset in range(0,len(order),8):
        ...
        steps+=1
        lr=.0001*min(1,steps/5)*(.1+.9*.5*(1+math.cos(math.pi*steps/step_total)))
    ...
    if val<best:
        best=val;model.save_pretrained(out/'adapter');tokenizer.save_pretrained(out/'adapter')
        (out/'selected-checkpoint.json').write_text(json.dumps({'epoch':epoch+1,'devLoss':val}))
if not (out/'adapter/adapter_model.safetensors').exists():raise RuntimeError('No checkpoint improved development loss')
# Merge selected checkpoint afresh, not necessarily final epoch.
del optim,model,base
import gc;gc.collect();torch.cuda.empty_cache()
from peft import PeftModel
clean=AutoModelForCausalLM.from_pretrained(root/'base',local_files_only=True,torch_dtype=torch.bfloat16)
merged=PeftModel.from_pretrained(clean,out/'adapter').merge_and_unload(safe_merge=True)
...
manifest.update({'completedAt':time.time(),'elapsedSeconds':time.monotonic()-start,'initialDevLoss':initial,'bestDevLoss':best,'peakAllocatedBytes':torch.cuda.max_memory_allocated(),'adapterHash':sha(out/'adapter/adapter_model.safetensors'),'mergedWeights':{p.name:sha(p) for p in (out/'merged').glob('*.safetensors')},'status':'trained_unvalidated','limitation':'Development loss on related templates is not proof of general capability improvement'})
