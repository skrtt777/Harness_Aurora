"""Bounded supervised LoRA training, v3 dataset (288 train / 40 dev — see
build-dataset-v3.mjs). Only assistant tokens contribute to loss. Original
base remains immutable; output contains actual trained adapter tensors.
Identical recipe to train-lora-v2.py (same base, same rank/lr/epochs
defaults) — the only deliberate change this round is a bigger, more diverse
dataset, so any effect measured is attributable to that, not a different
recipe too.
"""
...
parser.add_argument('--run', default='aurora-lora-v3')
...
model = get_peft_model(base, LoraConfig(r=8, lora_alpha=16, lora_dropout=.05, bias='none', task_type='CAUSAL_LM', target_modules=['q_proj','k_proj','v_proj','o_proj','gate_proj','up_proj','down_proj']))
...
manifest={'run':args.run,'base':json.loads((root/'base-provenance.json').read_text()),'seed':20260920,'epochs':args.epochs,'rank':8,'alpha':16,'learningRate':.00002,'chatFormat':'Exact Ollama generate ChatML, no implicit system message','batchSize':1,'gradientAccumulation':8,'maxSequenceLength':2048,'trainableParameters':trainable,'totalParameters':total,'trainHash':sha(root/'dataset-v3/train.jsonl'),'devHash':sha(root/'dataset-v3/dev.jsonl'),'trainingScriptHash':sha(Path(__file__)),'gpu':torch.cuda.get_device_name(0),'torch':torch.__version__,'startedAt':time.time()}
