...
import {heldout,retention} from './heldout-v2.mjs';
import {heldout as oldTasks,retention as oldRetention} from './heldout.mjs';
import {engineTasks} from '../benchmark/engine-tasks.mjs';
import {browserChecker} from './browser-check.mjs';
const root=resolve('reports/model-training-v2'),phase=process.argv[2],arm=process.argv[3];
const sha=x=>createHash('sha256').update(x).digest('hex'),read=p=>readFile(p,'utf8').then(JSON.parse);
const models={baseline:'qwen2.5-coder:1.5b',control:'aurora-control:1.5b-v1',e1:'aurora-local:1.5b-v2-e1',e2:'aurora-local:1.5b-v2-e2',candidate:'aurora-local:1.5b-v2'};
const files=['scripts/training/evaluate-v2.mjs','scripts/training/heldout-v2.mjs','scripts/training/heldout.mjs','scripts/training/curriculum.mjs','scripts/training/curriculum-v2.mjs','scripts/training/build-dataset-v2.mjs','scripts/training/train-lora-v2.py','scripts/training/browser-check.mjs','scripts/benchmark/engine-tasks.mjs',...(await readdir('app')).filter(f=>f.endsWith('.js')).map(f=>'app/'+f),...(await readdir('app/runtime-policy')).map(f=>'app/runtime-policy/'+f)];
...
process.env.HARNESS_DB_FILE=resolve('app/data/benchmarks/model-training-v2',phase+'-'+arm+'.db');await mkdir(dirname(process.env.HARNESS_DB_FILE),{recursive:true});
...
try{
 ...
 for(const t of tasks)for(const seed of seeds){
  ...
  const env={...process.env,LOCAL_SEED:String(seed)},c=await createConversation({provider:'local',title:`V2 ${phase}/${arm}/${t.id}/${seed}`});
