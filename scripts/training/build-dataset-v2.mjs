...
const out='models/local-training/dataset-v2';await mkdir(out,{recursive:false});
const sha=x=>createHash('sha256').update(x).digest('hex'),train=[],dev=[],checks=[],families=new Set();
const checker=await browserChecker();
try{
 for(let v=0;v<3;v++)for(const t of [...diverseCurriculum(v),...(v<2?curriculum(v):[])]){
  ...
  // Both actual Harness prompts and direct requests; no repetitive global CSS.
...
// Exact executable pure-function examples, not copied model answers.
const algorithms=[
...
await writeFile(out+'/manifest.json',JSON.stringify({createdAt:new Date().toISOString(),train:dedup.length,dev:dev.length,browserCases:checks.length,browserFamilies:[...families],algorithmFamilies:algorithms.length,algorithmCasesChecked:algorithms.reduce((s,x)=>s+x[3].length*3,0),categories:Object.fromEntries([...new Set(dedup.map(r=>r.kind))].map(k=>[k,dedup.filter(r=>r.kind===k).length])),trainHash:sha(await readFile(out+'/train.jsonl')),devHash:sha(await readFile(out+'/dev.jsonl')),origin:'Authored and verified locally. No final references, user conversations or external teacher API.'},null,2));
