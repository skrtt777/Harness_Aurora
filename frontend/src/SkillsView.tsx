...
export default function SkillsView() {
  ...
  const [hubFilter,setHubFilter]=useState({q:'',source:''});
  ...
  return <section className="skills-view"><h1>Skills e regras</h1>
    ...
    <section className="skills-hub" aria-label="Catálogo de skills"><div className="skills-hub-heading"><div><h2>Explore o catálogo Hermes</h2><p>{hub?.meta?`${hub.meta.total.toLocaleString('pt-BR')} entradas disponíveis para consulta`:'Carregue o índice para descobrir skills de várias fontes.'}</p></div><button disabled={busy} onClick={()=>void action(async()=>{await syncSkillCatalog();setHub(await searchSkillCatalog(hubFilter.q,hubFilter.source));})}>{busy?'Aguarde…':'Atualizar catálogo'}</button></div>
      ...
      <form className="skills-hub-search" onSubmit={e=>{e.preventDefault();void action(async()=>{const filter={q:hubQuery,source:hubSource};setHub(await searchSkillCatalog(filter.q,filter.source));setHubFilter(filter);});}}><label>Buscar no catálogo<input value={hubQuery} onChange={e=>setHubQuery(e.target.value)} placeholder="react, excel, game, testing…" maxLength={160}/></label><label>Fonte<select value={hubSource} onChange={e=>setHubSource(e.target.value)}><option value="">Todas as fontes</option>{hub?.sources.map(s=><option key={s.source} value={s.source}>{s.source} ({s.count.toLocaleString('pt-BR')})</option>)}</select></label><button disabled={busy||!hub?.meta}>Buscar</button></form>
      {hub?.meta&&<><small>{hub.total.toLocaleString('pt-BR')} resultados · índice atualizado em {new Date(hub.meta.syncedAt).toLocaleDateString('pt-BR')}</small><div className="skill-grid catalog-grid">{hub.skills.map(s=><article key={s.id}><small>{s.source}</small><h3>{s.name}</h3><p>{s.description||'A fonte não forneceu descrição.'}</p><button disabled={busy||!s.importable} onClick={()=>void action(async()=>{const imported=await importCatalogSkill(s.id);setReference('');setSelected(await readSkill(imported.id));})}>Importar para revisão</button></article>)}</div>{!hub.skills.length&&<p>Nenhuma skill encontrada. Experimente outro termo.</p>}<div className="catalog-pagination"><button disabled={busy||hub.page===0} onClick={()=>void action(async()=>setHub(await searchSkillCatalog(hubFilter.q,hubFilter.source,hub.page-1)))}>Anterior</button><span>Página {hub.page+1} de {Math.max(1,Math.ceil(hub.total/hub.limit))}</span><button disabled={busy||(hub.page+1)*hub.limit>=hub.total} onClick={()=>void action(async()=>setHub(await searchSkillCatalog(hubFilter.q,hubFilter.source,hub.page+1)))}>Próxima</button></div></>}
    </section>
