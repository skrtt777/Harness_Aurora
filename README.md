...
- **O modelo local se revisa sozinho antes de responder** (`app/localRefine.js`, sempre com compute local/grátis, nunca Codex/Claude): quando a resposta parece código, uma checagem de sintaxe (`node --check`) mais um heurístico específico para variáveis `const` reatribuídas (um erro de runtime comum nesse modelo, que a checagem de sintaxe sozinha não pega) tentam uma correção automática; depois, uma segunda chamada ao próprio modelo revisa a resposta contra as memórias relevantes já selecionadas, e só adota a revisão se ela ainda contiver código de verdade (evita o modelo trocar código por um resumo em prosa).
